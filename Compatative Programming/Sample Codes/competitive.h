...
void init_st(int N) { // if original array size is N,
  // the required segment_tree array length is 2*2^(floor(log2(N)) + 1);
  int length = (int)(2 * pow(2.0, floor((log((double)N) / log(2.0)) + 1)));
  st.resize(length, 0); // resize this vector and fill with 0
}

void build_st(int code, int A[], int node, int b, int e) {
  if (b == e) { // as b == e, either one is fine
    if (code == RANGE_SUM) st[node] = A[b]; // store value of this cell
    else                   st[node] = b; // if RANGE_MIN/MAXIMUM, store index
  }
  else { // recursively compute the values in the left and right subtrees
    int leftIdx = 2 * node, rightIdx = 2 * node + 1;
    build_st(code, A, leftIdx , b              , (b + e) / 2);
    build_st(code, A, rightIdx, (b + e) / 2 + 1, e          );
    int lContent = st[leftIdx], rContent = st[rightIdx];
    
    if (code == RANGE_SUM) // make this segment contains sum of left and right subtree
      st[node] = lContent + rContent;
    else { // (code == RANGE_MIN/MAXIMUM)
      int lValue = A[lContent], rValue = A[rContent];
      if (code == RANGE_MIN) st[node] = (lValue <= rValue) ? lContent : rContent;
      else                   st[node] = (lValue >= rValue) ? lContent : rContent;
} } }

int query(int code, int A[], int node, int b, int e, int i, int j) {
  if (i > e || j < b) return -1; // if current interval outside query interval 
  if (b >= i && e <= j) return st[node]; // if current interval inside query interval
...
int update_point(int code, int A[], int node, int b, int e, int idx, int new_value) {
  ...
  if (b == i && e == j) {
    ...
    if (code == RANGE_SUM)
      return st[node] = A[b]; // this new value
    else
